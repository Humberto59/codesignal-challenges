...
def build_binary_tree(arg=None):
    """
    Build a binary tree from a list, where all the values are present
    from top to bottom level, from left to rigth.
    """
    root = Binary_node(arg[0])
    pool = [root]
    indx = 1
    att = ['left', 'right']
    _li = [0, 1]
...
def print_binary_tree(root=None):
...
class Ternary_node(object):
    """ Class for a node of a Ternary Tree """
    def __init__(self, value):
        self.value = value
        self.left = None
        self.middle = None
